...
import { 
  Users, Bed, Bath, Wifi, Wind, MapPin, 
  Tv, Coffee, Flame, Trees, Compass, CheckCircle2, Calendar, Sparkles 
} from 'lucide-react';
...
interface CottagesProps {
  ...
  cmsData: CMSData;
  onBookRequest: (bookingData: Omit<Booking, 'id' | 'createdAt' | 'status'>) => void;
}

export default function Cottages({ currentLang, cmsData, onBookRequest }: CottagesProps) {
  ...
  const isDateBlocked = (cottageId: string, dateStr: string) => {
    ...
    const cottageBookings = cmsData.bookings.filter(b => b.cottageId === cottageId && b.status === 'Confirmed');
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    if (!checkIn || !checkOut) {
      setFormError(currentLang === 'ge' ? 'გთხოვთ მიუთითოთ თარიღები' : 'Please specify check-in & check-out dates');
      return;
    }

    const { nights, totalPrice } = calculateBookingDetails(selectedCottage);
    if (nights <= 0) {
      setFormError(currentLang === 'ge' ? 'გასვლის თარიღი უნდა იყოს შესვლის თარიღის შემდეგ!' : 'Check-out date must be after check-in date!');
      return;
    }
    ...
    if (datesAreBlocked) {
      setFormError(currentLang === 'ge' ? 'სამწუხაროდ, არჩეული პერიოდი უკვე დაკავებულია!' : 'Sorry, some dates in your selected range are already booked!');
      return;
    }

    if (!customerName || !customerPhone) {
      setFormError(currentLang === 'ge' ? 'სახელი და ტელეფონი სავალდებულოა' : 'Name and Phone are required');
      return;
    }
  ...
  return (
    <section id="cottages" className="py-24 bg-stone-950 text-white scroll-mt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="text-center mb-16">
          <span className="text-amber-400 text-xs font-semibold uppercase tracking-[0.2em]">
            {currentLang === 'ge' ? 'ჩვენი საცხოვრებლები' : 'PREMIUM ACCOMMODATION'}
          </span>
          <h2 className="font-serif text-3xl md:text-5xl font-bold tracking-tight mt-2">
            {currentLang === 'ge' ? 'აღმოაჩინეთ ილიასეულის კოტეჯები' : 'Explore Our Luxury Wooden Cottages'}
          </h2>
        ...
        <div className="space-y-16">
          {cmsData.cottages.map((cottage, index) => {
            ...
            const cottageName = currentLang === 'ge' ? cottage.name.ge : cottage.name.en;
            const cottageDesc = currentLang === 'ge' ? cottage.description.ge : cottage.description.en;
            ...
            return (
              ...
              >
                ...
                <div className={`lg:col-span-6 relative h-75 md:h-105 rounded-2xl overflow-hidden group shadow-lg ${
                  ...
                  {cottage.images.length > 1 && (
                    <>
                      <button 
                        onClick={() => prevImage(cottage.id, cottage.images.length)}
                      ...
                      <button 
                        onClick={() => nextImage(cottage.id, cottage.images.length)}
                      ...
                      {cottage.images.map((_, imgIdx) => (
                        <span 
                          ...
                          className={`w-1.5 h-1.5 rounded-full transition-colors ${
                            imgIdx === activeImgIdx ? 'bg-amber-400' : 'bg-stone-500'
                          }`}
                        />
                      ))}
                    </div>
                ...
                <div className="lg:col-span-6 flex flex-col justify-between h-full space-y-6">
                  <div>
                    ...
                    <div className="flex items-center space-x-2 text-amber-400 text-xs font-semibold uppercase tracking-widest mb-2">
                      <Sparkles size={14} />
                      <span>{currentLang === 'ge' ? 'უმაღლესი კლასის კომფორტი' : 'PREMIUM LIVING'}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 py-4 px-4 bg-stone-900/80 rounded-xl border border-stone-800 text-center mb-6">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-stone-400 uppercase tracking-wider">{t.capacity}</span>
                        <span className="text-sm font-semibold text-white">{cottage.guests} {currentLang === 'ge' ? 'სტუმარი' : 'Guests'}</span>
                      </div>
                      <div className="flex flex-col items-center border-x border-stone-800">
                        <Bed className="text-amber-400 mb-1" size={18} />
                        <span className="text-[10px] text-stone-400 uppercase tracking-wider">{t.bedrooms}</span>
                        <span className="text-sm font-semibold text-white">{cottage.bedrooms}</span>
                      </div>
                      <div className="flex flex-col items-center">
                        <Bath className="text-amber-400 mb-1" size={18} />
                        <span className="text-[10px] text-stone-400 uppercase tracking-wider">{t.bathrooms}</span>
                        <span className="text-sm font-semibold text-white">{cottage.bathrooms}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {cottage.amenities.slice(0, 8).map((amenityKey) => (
                        ...
                        >
                          {amenityIcons[amenityKey] || <CheckCircle2 size={12} />}
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-stone-800 pt-6">
                    <div>
                      <span className="text-xs text-stone-400 block uppercase tracking-wider">{currentLang === 'ge' ? 'ღირებულება' : 'Price per night'}</span>
                      <div className="flex items-baseline space-x-1">
                        <span className="text-3xl font-serif font-semibold text-amber-300">{cottage.price} ₾</span>
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-stone-800 pb-4 mb-6">
                  <div>
                    ...
                    <p className="text-xs text-stone-400">
                      {currentLang === 'ge' ? selectedCottage.name.ge : selectedCottage.name.en}
                    </p>
                  </div>
                  <button 
                ...
                <form onSubmit={handleBookingSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.checkIn}</label>
                      <input 
                        type="date"
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.checkOut}</label>
                      <input 
                        type="date"
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.guests}</label>
                      <select 
                        value={guestsCount}
                      ...
                      >
                        {[...Array(selectedCottage.guests)].map((_, i) => (
                          <option key={i+1} value={i+1} className="bg-stone-900">{i+1} {currentLang === 'ge' ? 'სტუმარი' : 'Guests'}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.fullName}</label>
                      <input 
                        type="text"
                        value={customerName}
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.phone}</label>
                      <input 
                        type="tel"
                        value={customerPhone}
                  ...
                  <div>
                    <label className="block text-xs uppercase tracking-wider text-stone-400 mb-1 font-medium">{t.notes}</label>
                    <textarea 
                      value={customerNotes}
                      onChange={(e) => setCustomerNotes(e.target.value)}
                      rows={3}
                      placeholder={currentLang === 'ge' ? 'სურვილები, ჩამოსვლის დრო და სხვ...' : 'Arrival time, requests, grocery help...'}
                  ...
                  {checkIn && checkOut && (
                    <div className="bg-stone-950 p-4 rounded-xl border border-stone-800">
                      <div className="flex justify-between items-center text-sm mb-1.5 text-stone-400">
                        <span>{selectedCottage.price} ₾ × {calculateBookingDetails(selectedCottage).nights} {t.nights}</span>
                        <span>{calculateBookingDetails(selectedCottage).totalPrice} ₾</span>
                      </div>
                      <div className="flex justify-between items-center text-base font-bold border-t border-stone-800 pt-2 text-white">
                        <span>{t.totalPrice}</span>
                        <span className="text-lg text-amber-300">{calculateBookingDetails(selectedCottage).totalPrice} ₾</span>
                      </div>
                    </div>
                  )}
